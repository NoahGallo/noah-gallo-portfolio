...
export function CertificationsSection() {
  ...
  const { ref: currentRef, hasIntersected: currentVisible } = useIntersectionObserver()
  const { ref: upcomingRef, hasIntersected: upcomingVisible } = useIntersectionObserver()
  const { ref: goalRef, hasIntersected: goalVisible } = useIntersectionObserver()
            ...
            {upcomingCerts.map((cert, index) => (
              <div 
                ...
                className={`bg-gray-50 dark:bg-gray-800 rounded-lg p-6 border-2 border-dashed border-gray-300 dark:border-gray-600 hover:border-blue-400 dark:hover:border-blue-500 transition-all duration-300 group hover:scale-105 ${
                  upcomingVisible ? 'animate-fadeInUp' : 'opacity-0 translate-y-4'
                }`}
              ...
              >
                <div className="text-center">
                  <div className="w-16 h-16 bg-gradient-to-br from-blue-100 to-purple-100 dark:from-blue-900/30 dark:to-purple-900/30 rounded-full flex items-center justify-center mx-auto mb-4 group-hover:scale-110 transition-transform">
                    <span className="text-2xl group-hover:animate-bounce">{cert.logo}</span>
                  </div>
                  ...
                  <span className={`inline-block px-3 py-1 rounded-full text-xs font-medium transition-all duration-300 hover:scale-105 ${
